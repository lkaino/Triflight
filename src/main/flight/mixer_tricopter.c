        ...
        int16_t *pTailServo)
{
    ...
    tailServoThrustFactor = gpTriMixerConfig->tri_tail_motor_thrustfactor / 10.0f;
    tailServoMaxAngle = gpTriMixerConfig->tri_servo_angle_at_max * 10;
    tailServoSpeed = gpTriMixerConfig->tri_tail_servo_speed;
    ...
    motorAcceleration = (float)throttleRange / tri_motor_acceleration_float;

    // Reset the I term when motor deceleration has lasted 65% of the min to max time
    resetITermDecelerationLasted_ms = (uint16_t)(gpTriMixerConfig->tri_motor_acceleration * 1000.0f * 0.65f);
...
void triServoMixer(int16_t PIDoutput)
{
    ...
    if (gpTriMixerConfig->tri_servo_feedback != TRI_SERVO_FB_VIRTUAL)
    {
        ...
        tailServoADC = pt1FilterApply4(&feedbackFilter, adcGetChannel(tailServoADCChannel), 70, getdT());
    }
...
{
    return ((gpTriMixerConfig->tri_unarmed_servo != 0) || FLIGHT_MODE(TAILTUNE_MODE));
}
...
STATIC_UNIT_TESTED void tailTuneModeThrustTorque(thrustTorque_t *pTT, const bool isThrottleHigh)
{
    ...
    case TT_ACTIVE:
        if (isThrottleHigh &&
            ...
            (fabsf(gyro.gyroADCf[FD_YAW]) <= 5.0f)) // deg/s
        {
    ...
    case TT_WAIT_FOR_DISARM:
        if (!ARMING_FLAG(ARMED))
        {
            ...
            if (averageServoAngle > 90.5f && averageServoAngle < 120.f) {
                ...
                gpTriMixerConfig->tri_tail_motor_thrustfactor = 10.0f * cos_approx(averageServoAngle) / sin_approx(averageServoAngle);
...
static void tailTuneModeServoSetup(struct servoSetup_t *pSS, servoParam_t *pServoConf, int16_t *pServoVal)
{
    ...
    case SS_SETUP:
        ...
        {
            pSS->servoVal += triServoDirection() * -1.0f * (float)rcCommand[YAW] * getdT();
    ...
    case SS_CALIB:
        ...
        case SS_C_CALIB_SPEED:
            ...
            case SS_C_MIN:
                ...
                if (tailServoADC < (gpTriMixerConfig->tri_servo_min_adc + 10))
                {
            ...
            case SS_C_MAX:
                ...
                if (tailServoADC > (gpTriMixerConfig->tri_servo_max_adc - 10))
                {
                    if (!pSS->cal.waitingServoToStop)
                    {
                        pSS->cal.avg.sum += millis() - pSS->cal.timestamp_ms;
...
static void updateServoAngle(void)
{
    if (gpTriMixerConfig->tri_servo_feedback == TRI_SERVO_FB_VIRTUAL)
    {
        tailServoAngle = virtualServoStep(tailServoAngle, tailServoSpeed, getdT(), gpTailServoConf, *gpTailServo);
    }
    else
    {
        tailServoAngle = feedbackServoStep(gpTriMixerConfig, tailServoADC);
    }
}
...
static void tailMotorStep(int16_t setpoint, float dT)
{
    ...
    tailMotorVirtual = pt1FilterApply4(&motorFilter, current, 20, dT);
}
